-- Create VX Pay ("Escrowed Payment Agreement") tables on the Netlify Database.
...
-- + Veridex signature, so a VX Pay agreement verifies through the same
-- /verify/:id surface.

CREATE TABLE IF NOT EXISTS vxpay_agreements (
  id TEXT PRIMARY KEY,                            -- VXPAY-YYYY-XXXXXXXX
  user_id BIGINT,                                 -- creator's user id (auth lives in Supabase)
  ...
  -- Optional on-chain settlement binding (null for off-chain agreements)
  ...
  -- Funding (buyer confirms funds placed)
