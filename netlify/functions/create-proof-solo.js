...
import { handler as authSessionHandler } from './auth-session.js';
...
// Reuse the existing auth-session handler to verify the session
async function verifySession(event) {
  try {
    const cookie = event.headers.cookie || event.headers.Cookie || '';
    const result = await authSessionHandler({
      httpMethod: 'GET',
...
export const handler = async (event) => {
  ...
  // 1. Verify authentication using existing auth-session handler
